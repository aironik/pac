...
void Romb::generateVertexes() {
    ...
    const GLKVector3 topVertex = GLKVector3Make(0.0f, -1.0f , 0.0f);
    const GLKVector3 bottomVertex = GLKVector3Make(0.0f, 1.0f , 0.0f);
    for (GLuint i = 0; i < sectionsCount; ++i) {
        ...
        vertexDataPtr[1] = vertex0;
        vertexDataPtr[2] = vertex1;

        GLKVector3 normal1 = GLKVector3CrossProduct(GLKVector3Subtract(vertex0, vertex1),
                                                   GLKVector3Subtract(topVertex, vertex1));
        vertexDataPtr[3] = normal1;
        vertexDataPtr[4] = normal1;
        vertexDataPtr[5] = normal1;

        vertexDataPtr += 6;

        vertexDataPtr[0] = bottomVertex;
        vertexDataPtr[1] = vertex1;
        vertexDataPtr[2] = vertex0;

        GLKVector3 normal2 = GLKVector3CrossProduct(GLKVector3Subtract(vertex1, vertex0),
                                                   GLKVector3Subtract(bottomVertex, vertex0));
        vertexDataPtr[3] = normal2;
        vertexDataPtr[4] = normal2;
